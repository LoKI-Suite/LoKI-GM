    ...
    methods
        function gui = Input_GUI(inputFile)
            ...
            if nargin < 1 || isempty(inputFile)
                ...
                try
                    ...
                    warning(ME.identifier, 'Error loading input file: %s. Using default setup.', ME.message);
        ...
        function createWorkingConditionsPanel(gui, parent)
            ...
            gui.UIControls.workingConditions.excitationFrequency = uieditfield(grid, 'numeric', ...
                ...
                'ValueChangedFcn', @(src, evt) gui.handleExcitationFrequencyChange(evt.Value));
            ...
            % Gas Temperature [cite: 1] - Always active, no checkbox
            ...
            wallTempCheckbox = uicheckbox(grid, 'Text', 'Wall Temperature (K):', ...
                'ValueChangedFcn', @(src, evt) gui.toggleWallTemperatureEnable(evt.Value));
            wallTempCheckbox.Layout.Row = row;
            wallTempCheckbox.Layout.Column = 1;
            gui.UIControls.workingConditions.wallTemperatureCheckbox = wallTempCheckbox;
            ...
            extTempCheckbox = uicheckbox(grid, 'Text', 'External Temperature (K):', ...
                'ValueChangedFcn', @(src, evt) gui.toggleExternalTemperatureEnable(evt.Value));
            extTempCheckbox.Layout.Row = row;
            extTempCheckbox.Layout.Column = 1;
            gui.UIControls.workingConditions.extTemperatureCheckbox = extTempCheckbox;
            ...
            surfaceSiteDensityCheckbox = uicheckbox(grid, 'Text', 'Surface Site Density (m⁻²):', ...
            ...
            electronDensityCheckbox = uicheckbox(grid, 'Text', 'Electron Density (m⁻³):', ...
            ...
            chamberLengthCheckbox = uicheckbox(grid, 'Text', 'Chamber Length (m):', ...
                'ValueChangedFcn', @(src, evt) gui.toggleChamberLengthEnable(evt.Value));
            chamberLengthCheckbox.Layout.Row = row;
            chamberLengthCheckbox.Layout.Column = 1;
            gui.UIControls.workingConditions.chamberLengthCheckbox = chamberLengthCheckbox;
            ...
            chamberRadiusCheckbox = uicheckbox(grid, 'Text', 'Chamber Radius (m):', ...
                'ValueChangedFcn', @(src, evt) gui.toggleChamberRadiusEnable(evt.Value));
            chamberRadiusCheckbox.Layout.Row = row;
            chamberRadiusCheckbox.Layout.Column = 1;
            gui.UIControls.workingConditions.chamberRadiusCheckbox = chamberRadiusCheckbox;
        ...
        function createElectronKineticsPanel(gui, parent)
            ...
            filesGrid.RowHeight = {'1x', 45, 45}; % LXCat Files grows with window, others fixed ~1 item
        ...
        function createGasPropertiesPanel(gui, parent)
            ...
            % --- Gas Fractions ---
            fractionsPanel = uipanel(grid, ...
                'Title', 'Gas Fractions', ...
                'Tooltip', sprintf(['(?) Format: one gas per line, e.g.,\n', ...
                '\n',...
                'N2 = 0.07\n', ...
                'N = 0\n', ...
                'H2 = 0.92\n', ...
                ...
                'NH3 = 0.0\n', ...
                '\n',...
                'The total sum of all fractions must equal 1.']));
            fractionsPanel.Layout.Row = 2;
            fractionsGrid = uigridlayout(fractionsPanel, [1, 2]);
            fractionsGrid.ColumnWidth = {'9x', '1x'}; % Text area gets 90%, button gets 10%
            fractionsGrid.RowHeight = {'1x'};
            fractionsGrid.Padding = [10 10 10 10];
            fractionsGrid.RowSpacing = 5;
            fractionsGrid.ColumnSpacing = 10;
            
            % Text area for gas fractions (editable text field) - uses theme colors
            gui.UIControls.gasProperties.fraction = uitextarea(fractionsGrid, ...
                'Value', {''}, ...
                'Editable', 'off', ...
                'FontName', 'Consolas');
            gui.UIControls.gasProperties.fraction.Layout.Row = 1;
            gui.UIControls.gasProperties.fraction.Layout.Column = 1;
            
            % Edit/OK button - wider now (1/3 of width)
            gui.UIControls.gasProperties.fractionEditBtn = uibutton(fractionsGrid, ...
                'Text', 'Edit', ...
                'ButtonPushedFcn', @(src,evt) gui.toggleGasFractionEdit());
            gui.UIControls.gasProperties.fractionEditBtn.Layout.Row = 1;
            gui.UIControls.gasProperties.fractionEditBtn.Layout.Column = 2;
        end
        ...
        function populateGUIFromSetup(gui)
            ...
            try
                ...
                warning(ME.identifier, 'Error setting LXCatFiles list: %s', ME.message);
            end
            try
                % Check if LXCatExtraFiles exists in setup
                if isfield(gui.Setup.electronKinetics, 'LXCatExtraFiles')
                    extraFiles = gui.Setup.electronKinetics.LXCatExtraFiles;
                    if iscell(extraFiles)
                        gui.UIControls.electronKinetics.LXCatExtraFiles.Items = extraFiles;
                        ...
                        gui.UIControls.electronKinetics.LXCatExtraFiles.Items = {};
                    end
                else
                    % Field doesn't exist, set empty
                    gui.UIControls.electronKinetics.LXCatExtraFiles.Items = {};
                end
            catch ME
                warning(ME.identifier, 'Error setting LXCatExtraFiles list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error setting effectiveCrossSectionPopulations list: %s', ME.message);
            end
            try
                gui.UIControls.gasProperties.fraction.Value = gui.Setup.electronKinetics.gasProperties.fraction;
            catch ME
                warning(ME.identifier, 'Error setting gas fractions list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error setting CARgases list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error setting state populations list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error setting state energy list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error setting state statistical weight list: %s', ME.message);
            end
            try
                ...
                warning(ME.identifier, 'Error configuring data sets checkboxes: %s', ME.message);
            end

            % Configure initial state of smart grid controls
            try
                smartGridIsOn = gui.Setup.electronKinetics.numerics.energyGrid.smartGrid.isOn;
                % Update the checkbox value
                gui.UIControls.electronKinetics.numerics.energyGrid.smartGrid.isOn.Value = smartGridIsOn;
                % Enable/disable related controls
                gui.toggleSmartGridEnable(smartGridIsOn);
            catch ME
                warning(ME.identifier, 'Error configuring smart grid controls: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring advanced button visibility: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring ODE parameters: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring GUI controls: %s', ME.message);
            end

            % Configure initial state of Working Conditions optional fields (inactive by default)
            % Note: Gas Temperature is always active, no checkbox needed
            try
                ...
                gui.toggleWallTemperatureEnable(false);
                gui.toggleExternalTemperatureEnable(false);
                gui.toggleChamberLengthEnable(false);
                gui.toggleChamberRadiusEnable(false);
            catch ME
                warning(ME.identifier, 'Error configuring optional working conditions: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring electron kinetics controls: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring output controls: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring SCCM Outflow type: %s', ME.message);
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring EEDF type visibility: %s', ME.message);
            end
        ...
        function setControlValue(gui, control, dataPath)
            ...
            if contains(dataPath, 'tabs.tabGroup') || ...
               ...
               contains(dataPath, 'wallTemperatureCheckbox') || ...
               contains(dataPath, 'extTemperatureCheckbox') || ...
               contains(dataPath, 'chamberLengthCheckbox') || ...
               contains(dataPath, 'chamberRadiusCheckbox') || ...
               ...
               contains(dataPath, 'fractionEditBtn')
                return;
            end
            ...
            try
                ...
                elseif isa(control, 'matlab.ui.control.TextArea')
                    % For text areas (like gas fractions), set Value as cell array
                    if iscell(value)
                        control.Value = value;
                    elseif ischar(value) || isstring(value)
                ...
                warning(ME.identifier, 'Error setting control value for path %s: %s', dataPath, ME.message);
            end
        end
        ...
        function updateField(gui, control, fieldPath, value)
            ...
            try
                ...
                warning(ME.identifier, 'Error updating field "%s": %s', fieldPath, ME.message);
        ...
        function handleAlgorithmChange(gui, algorithmValue)
            ...
            try
                ...
                warning(ME.identifier, 'Error handling algorithm change: %s', ME.message);
            end
        end
        ...
        function sliderMixingParameterChanged(gui, ~, value)
            ...
            try
                ...
                warning(ME.identifier, 'Error updating mixing parameter from slider: %s', ME.message);
            end
        end
        ...
        function textMixingParameterChanged(gui, ~, value)
            ...
            try
                ...
                warning(ME.identifier, 'Error updating mixing parameter from text field: %s', ME.message);
            end
        end
        ...
        function addListItem(gui, fieldPath, browseFile)
            ...
            if ~isempty(newItem) && ~ismember(newItem, currentItems)
                % For gas fractions, validate the format only (no automatic correction)
                if endsWith(fieldPath, 'gasProperties.fraction') || contains(fieldPath, 'gasProperties.fraction')
                    ...
                    % Note: We do NOT automatically correct the sum here
                    % User must ensure sum = 1 manually when they edit/check the fractions
                end
        ...
        function editListItem(gui, listBox, fieldPath)
            ...
            % Single-line edit dialog for list items
            ...
            if length(currentItems) == 1
                listBox.Value = currentItems{1};
            else
                listBox.Value = {}; % Clear selection if multiple items
            end
        ...
        function handleEECollisionsChange(gui, value)
            ...
            % Gas Temperature is always enabled, regardless of e-e collisions state
        end

        function handleExcitationFrequencyChange(gui, value)
            % Update Setup
            gui.updateField(gui.UIControls.workingConditions.excitationFrequency, 'workingConditions.excitationFrequency', value);
            
            % When excitation frequency is different from zero, activate Gas Pressure checkbox
            % When frequency is zero, deactivate Gas Pressure checkbox
            if value ~= 0
                gui.UIControls.workingConditions.gasPressureCheckbox.Value = true;
        ...
        function toggleGasFractionEdit(gui)
            % Toggle between Edit and OK mode for gas fractions text area
            btn = gui.UIControls.gasProperties.fractionEditBtn;
            textarea = gui.UIControls.gasProperties.fraction;
            
            if strcmp(btn.Text, 'Edit')
                % Enter edit mode
                ...
                for i = 1:length(lines)
                    ...
                    if isempty(match)
                        uialert(gui.Fig, sprintf('Invalid format on line %d: "%s"\nExpected: "species = number" (e.g., "N2 = 0.8")', i, line), 'Invalid Format');
                        return;
                    end
                    
                    fraction = str2double(match{1}{2});
                    if isnan(fraction) || fraction < 0 || fraction > 1
                        uialert(gui.Fig, sprintf('Invalid fraction value on line %d: "%s"', i, line), 'Invalid Value');
                        return;
                    end
                    
                    totalFraction = totalFraction + fraction;
                    validEntries{end+1} = line;
                end
                
                % Checksum validation
                if abs(totalFraction - 1.0) > 1e-6
                    uialert(gui.Fig, sprintf('Gas fractions must sum to 1.0\nCurrent sum: %.6f', totalFraction), 'Invalid Sum');
                    return;
                end
                
                % Validation passed - exit edit mode
                textarea.Value = validEntries';
                textarea.Editable = 'off';
                btn.Text = 'Edit';
                
                % Update setup
                gui.Setup.electronKinetics.gasProperties.fraction = validEntries';
            end
        end
        ...
        function toggleWallTemperatureEnable(gui, isEnabled)
            % Enable/disable wall temperature based on checkbox state
            if isEnabled
                gui.UIControls.workingConditions.wallTemperature.Enable = 'on';
            else
                gui.UIControls.workingConditions.wallTemperature.Enable = 'off';
            end
        end

        function toggleExternalTemperatureEnable(gui, isEnabled)
            % Enable/disable external temperature based on checkbox state
            if isEnabled
                gui.UIControls.workingConditions.extTemperature.Enable = 'on';
            else
                gui.UIControls.workingConditions.extTemperature.Enable = 'off';
            end
        end

        function toggleChamberLengthEnable(gui, isEnabled)
            % Enable/disable chamber length based on checkbox state
            if isEnabled
                gui.UIControls.workingConditions.chamberLength.Enable = 'on';
            else
                gui.UIControls.workingConditions.chamberLength.Enable = 'off';
            end
        end

        function toggleChamberRadiusEnable(gui, isEnabled)
            % Enable/disable chamber radius based on checkbox state
            if isEnabled
                gui.UIControls.workingConditions.chamberRadius.Enable = 'on';
            else
                gui.UIControls.workingConditions.chamberRadius.Enable = 'off';
            end
        end
        ...
        function loadSettings(gui, ~, ~)
            ...
            try
                ...
                % Force certain fields to always be disabled
                gui.forceDisableAlwaysOffFields();
                % Validate gas fractions sum
                gui.validateGasFractionsOnLoad();
        ...
        function [setup, foundFields] = parseInputFile(gui, inputFile)
            ...
            while i <= numel(lines)
                ...
                key = strtrim(tok{1});
                originalKey = key; % Store original name before normalization
                valStr = strtrim(tok{2}); % Trim value string to remove trailing spaces from inline comments
                
                % Normalize field names (handle different naming conventions in .in files)
                % Case-insensitive comparison
                keyLower = lower(key);
                if strcmp(keyLower, 'lxcatfilesextra')
                    key = 'LXCatExtraFiles'; % Convert to internal name
                elseif strcmp(keyLower, 'effectivecrosssectionpopulations')
                    key = 'effectiveCrossSectionPopulations'; % Ensure correct casing
                elseif strcmp(keyLower, 'cargases')
                    key = 'CARgases'; % Ensure correct casing
                end
                ...
                if isempty(valStr)
                    ...
                    if isList
                        ...
                        if ~strcmp(originalKey, key)
                            originalPathParts = [keyStack, {originalKey}];
                            originalFieldPath = strjoin(originalPathParts, '.');
                            foundFields(originalFieldPath) = true;
                        end
                        ...
                        % Also mark with original name if it was normalized (for compatibility)
                        if ~strcmp(originalKey, key)
                            originalPathParts = [keyStack(1:end-1), {originalKey}];
                            originalFieldPath = strjoin(originalPathParts, '.');
                            foundFields(originalFieldPath) = true;
                        end
                    end
        ...
        function setup = convertJSONToSetup(gui, jsonData, setup, foundFields, prefix)
            ...
            for i = 1:length(fields)
                ...
                if strcmp(fieldNameLower, 'lxcatfilesextra')
                    fieldName = 'LXCatExtraFiles'; % Convert to internal name
                elseif strcmp(fieldNameLower, 'effectivecrosssectionpopulations')
                    fieldName = 'effectiveCrossSectionPopulations'; % Ensure correct casing
                elseif strcmp(fieldNameLower, 'cargases')
                    fieldName = 'CARgases'; % Ensure correct casing
                end
        ...
        function generateJSONFile(gui, filename)
            ...
            try
                ...
                warning(ME.identifier, 'Error updating totalSccmOutFlow from UI: %s', ME.message);
            end
        ...
        function jsonStruct = convertSetupToJSON(gui, setup)
            ...
            if isfield(jsonStruct, 'workingConditions')
                ...
                % Note: gasTemperature is always included (always active, no checkbox)
                ...
                % wallTemperature - only if checkbox is checked
                if isfield(gui.UIControls.workingConditions, 'wallTemperatureCheckbox')
                    if ~gui.UIControls.workingConditions.wallTemperatureCheckbox.Value
                        if isfield(jsonStruct.workingConditions, 'wallTemperature')
                            jsonStruct.workingConditions = rmfield(jsonStruct.workingConditions, 'wallTemperature');
                        end
                    end
                end
                
                % extTemperature - only if checkbox is checked
                if isfield(gui.UIControls.workingConditions, 'extTemperatureCheckbox')
                    if ~gui.UIControls.workingConditions.extTemperatureCheckbox.Value
                        if isfield(jsonStruct.workingConditions, 'extTemperature')
                            jsonStruct.workingConditions = rmfield(jsonStruct.workingConditions, 'extTemperature');
                        end
                    end
                end
                
                % chamberLength - only if checkbox is checked
                if isfield(gui.UIControls.workingConditions, 'chamberLengthCheckbox')
                    if ~gui.UIControls.workingConditions.chamberLengthCheckbox.Value
                        if isfield(jsonStruct.workingConditions, 'chamberLength')
                            jsonStruct.workingConditions = rmfield(jsonStruct.workingConditions, 'chamberLength');
                        end
                    end
                end
                
                % chamberRadius - only if checkbox is checked
                if isfield(gui.UIControls.workingConditions, 'chamberRadiusCheckbox')
                    if ~gui.UIControls.workingConditions.chamberRadiusCheckbox.Value
                        if isfield(jsonStruct.workingConditions, 'chamberRadius')
                            jsonStruct.workingConditions = rmfield(jsonStruct.workingConditions, 'chamberRadius');
                        end
                    end
                end
            end
        ...
        function disableUnusedOptionalFields(gui, foundFields)
            ...
            % Note: gasTemperature is always enabled (no checkbox, always active)
            % No need to check or toggle it
            ...
            if isFieldFound('workingConditions.wallTemperature')
                try
                    gui.toggleWallTemperatureEnable(true);
                    if isfield(gui.UIControls.workingConditions, 'wallTemperatureCheckbox')
                        gui.UIControls.workingConditions.wallTemperatureCheckbox.Value = true;
                    end
                catch
                end
            elseif ~isFieldFound('workingConditions.wallTemperature')
                try
                    gui.toggleWallTemperatureEnable(false);
                    if isfield(gui.UIControls.workingConditions, 'wallTemperatureCheckbox')
                        gui.UIControls.workingConditions.wallTemperatureCheckbox.Value = false;
                    end
                catch
                end
            end
            
            if isFieldFound('workingConditions.extTemperature')
                try
                    gui.toggleExternalTemperatureEnable(true);
                    if isfield(gui.UIControls.workingConditions, 'extTemperatureCheckbox')
                        gui.UIControls.workingConditions.extTemperatureCheckbox.Value = true;
                    end
                catch
                end
            elseif ~isFieldFound('workingConditions.extTemperature')
                try
                    gui.toggleExternalTemperatureEnable(false);
                    if isfield(gui.UIControls.workingConditions, 'extTemperatureCheckbox')
                        gui.UIControls.workingConditions.extTemperatureCheckbox.Value = false;
                    end
                catch
                end
            end
            
            if isFieldFound('workingConditions.chamberLength')
                try
                    gui.toggleChamberLengthEnable(true);
                    if isfield(gui.UIControls.workingConditions, 'chamberLengthCheckbox')
                        gui.UIControls.workingConditions.chamberLengthCheckbox.Value = true;
                    end
                catch
                end
            elseif ~isFieldFound('workingConditions.chamberLength')
                try
                    gui.toggleChamberLengthEnable(false);
                    if isfield(gui.UIControls.workingConditions, 'chamberLengthCheckbox')
                        gui.UIControls.workingConditions.chamberLengthCheckbox.Value = false;
                    end
                catch
                end
            end
            
            if isFieldFound('workingConditions.chamberRadius')
                try
                    gui.toggleChamberRadiusEnable(true);
                    if isfield(gui.UIControls.workingConditions, 'chamberRadiusCheckbox')
                        gui.UIControls.workingConditions.chamberRadiusCheckbox.Value = true;
                    end
                catch
                end
            elseif ~isFieldFound('workingConditions.chamberRadius')
                try
                    gui.toggleChamberRadiusEnable(false);
                    if isfield(gui.UIControls.workingConditions, 'chamberRadiusCheckbox')
                        gui.UIControls.workingConditions.chamberRadiusCheckbox.Value = false;
                    end
                catch
                end
            end
            ...
            % Smart Grid - disable if not found
            smartGridFound = isFieldFound('electronKinetics.numerics.energyGrid.smartGrid') || ...
            ...
            if ~smartGridFound
                try
                    if isfield(gui.UIControls.electronKinetics.numerics.energyGrid.smartGrid, 'isOn')
                        gui.UIControls.electronKinetics.numerics.energyGrid.smartGrid.isOn.Value = false;
            ...
            if ~isFieldFound('electronKinetics.gasProperties.fraction')
                ...
                try
                    if isfield(gui.UIControls, 'gasProperties') && isfield(gui.UIControls.gasProperties, 'fraction')
                        gui.UIControls.gasProperties.fraction.Value = {''};
                    end
                catch
                end
            end
        ...
        function activateFoundOptionalFields(gui, foundFields)
            ...
            if isFieldFound('electronKinetics.numerics.energyGrid.smartGrid') || ...
               ...
               isFieldFound('electronKinetics.numerics.energyGrid.smartGrid.updateFactor')
                try
                    ...
                    warning(ME.identifier, 'Error activating smartGrid: %s', ME.message);
                end
            end
            ...
            try
                ...
                warning(ME.identifier, 'Error configuring advanced options: %s', ME.message);
            end
            ...
            if isFieldFound('electronKinetics.effectiveCrossSectionPopulations')
                try
                    ...
                    warning(ME.identifier, 'Error activating effectiveCrossSectionPopulations: %s', ME.message);
                end
            end
            
            % LXCat Extra Files - if found, activate checkbox
            if isFieldFound('electronKinetics.LXCatExtraFiles') || isFieldFound('electronKinetics.LXCatFilesExtra')
                try
                    gui.toggleLXCatExtraEnable(true);
                    if isfield(gui.UIControls.electronKinetics, 'LXCatExtraCheckbox')
                        gui.UIControls.electronKinetics.LXCatExtraCheckbox.Value = true;
                    end
                catch ME
                    warning(ME.identifier, 'Error activating LXCatExtraFiles: %s', ME.message);
                end
            end
            
            % CAR Gases - if found, activate checkbox
            if isFieldFound('electronKinetics.CARgases')
                try
                    gui.toggleCARGasEnable(true);
                    if isfield(gui.UIControls.electronKinetics, 'CARcheckbox')
                    ...
                    warning(ME.identifier, 'Error activating CARgases: %s', ME.message);
                end
            end
        end

        function forceDisableAlwaysOffFields(gui)
            % Force certain fields to always be disabled regardless of input file
            % These fields are always off by default and should not be activated by loading
            
            alwaysDisabledFields = {
                'wallTemperature', 'wallTemperatureCheckbox', @gui.toggleWallTemperatureEnable;
                'externalTemperature', 'extTemperatureCheckbox', @gui.toggleExternalTemperatureEnable;
                'surfaceSiteDensity', 'surfaceSiteDensityCheckbox', @gui.toggleSurfaceSiteDensityEnable;
                'chamberLength', 'chamberLengthCheckbox', @gui.toggleChamberLengthEnable;
                'chamberRadius', 'chamberRadiusCheckbox', @gui.toggleChamberRadiusEnable;
                'totalSccmInFlow', 'totalSccmInFlowCheckbox', @gui.toggleTotalSccmInFlowEnable;
                'totalSccmOutFlow', 'totalSccmOutFlowCheckbox', @gui.toggleTotalSccmOutFlowEnable;
                'dischargeCurrent', 'dischargeCurrentCheckbox', @gui.toggleDischargeCurrentEnable;
                'dischargePowerDensity', 'dischargePowerCheckbox', @gui.toggleDischargePowerEnable;
            };
            
            for i = 1:size(alwaysDisabledFields, 1)
                checkboxName = alwaysDisabledFields{i, 2};
                toggleFunc = alwaysDisabledFields{i, 3};
                
                try
                    % Uncheck the checkbox
                    if isfield(gui.UIControls.workingConditions, checkboxName)
                        gui.UIControls.workingConditions.(checkboxName).Value = false;
                    end
                    % Disable the field
                    toggleFunc(false);
                catch
                    % Ignore errors for fields that may not exist
                end
            end
        end

        function validateGasFractionsOnLoad(gui)
            % Validate that gas fractions sum to 1.0 after loading a file
            % If not, show a warning and put the gas fractions in edit mode
            
            try
                fractions = gui.UIControls.gasProperties.fraction.Value;
                
                % Parse fractions and calculate sum
                pattern = '^\s*(\w+)\s*=\s*([0-9]+\.?[0-9]*)\s*$';
                totalSum = 0;
                hasValidFractions = false;
                
                for i = 1:length(fractions)
                    line = strtrim(fractions{i});
                    if isempty(line), continue; end
                    
                    match = regexp(line, pattern, 'tokens');
                    if ~isempty(match)
                        totalSum = totalSum + str2double(match{1}{2});
                        hasValidFractions = true;
                    end
                end
                
                % Only validate if there are fractions
                if hasValidFractions && abs(totalSum - 1.0) > 1e-6
                    % Show warning
                    uialert(gui.Fig, sprintf('Gas fractions do not sum to 1.0 (current sum: %.6f).\nPlease correct them using the Edit button.', totalSum), ...
                        'Invalid Gas Fractions', 'Icon', 'warning');
                    
                    % Put the gas fractions in edit mode
                    gui.UIControls.gasProperties.fraction.Editable = 'on';
                    gui.UIControls.gasProperties.fractionEditBtn.Text = 'OK';
                end
            catch
                % Ignore errors if gas fractions control doesn't exist
            end
        end
