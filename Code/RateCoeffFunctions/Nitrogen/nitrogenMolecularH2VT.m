function [rateCoeff, dependent] = nitrogenMolecularH2VT(~, ~, ~, reaction, ~, chemistry)
% nitrogenMolecularH2VT evaluates the rate coefficient for V-T transitions between nitrogen molecules and H2 described in:
...
% http://dx.doi.org/10.1088/
  ...
  Lmin = 3.5E-11;    % Minimum distance between N2 molecules during V-T collision (m)
  ...
  if (v-w)~=1  % check the reaction
    error(['hydrogenMolecularH2VT can not evaluate the rate coefficient for the reaction:\n%s\n' ...
  ...
  P10 = 5.63E-20*Tg*exp(-80.6*Tg^(-1/3));  %m3s-1
      
  Y = (1-2*chi*v)*(0.5^1.5)*2*pi*omega*Lmin*(uM/(kb*Tg))^0.5;
  if(Y>=0 & Y<=20)
  ...
  rateCoeff =v*(1-chi)/(1-chi*v)*P10*FY/FY1;
