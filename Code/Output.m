...
%        numberOfEbyNJobs = setup.pulseInfo.samplingPoints+1;
      ...
      % electronTemperature values on geid. If we study the chemistry, the
      % maintenance field is written in the end.
      % If any other parameter is an array, also write that array.
      if contains(output.dataFormat, 'hdf5')
        if strcmpi(setup.info.electronKinetics.eedfType, 'boltzmann')
          ...
          if ~setup.enableChemistry && isempty(setup.pulseInfo)
...
%               if output.isSimulationHF(output.currentJobID)
          ...
          case 'rateCoefficients'
            ...
            if contains(output.dataFormat, 'hdf5')
              ...
              if nxReactions > 0
                % get dims
                dims = [nxReactions 1 numberOfEbyNJobs output.extraDims];
                spaceID = H5S.create_simple(length(dims),fliplr(dims),[]);
...
%       elseif strcmp(output.dataFormat, 'hdf5') % in this case we don't need extra folders
      ...
      if output.chemParamsIsToBeSaved
          output.saveChemParams(chemistry.targetGasPressure, chemistry.workCond.struct, chemistry.caseDefinition, ...
...
%           name = ["Reactions","Rate("+rateUnitsStr+")", "Contribution"];
